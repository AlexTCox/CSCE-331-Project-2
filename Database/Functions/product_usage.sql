-- Description: This function generates a report of product usage within a specified time period.
...
-- Returns: Table containing the date and the count of product usage for each day within the specified time period.
...
BEGIN
    -- Query to retrieve the count of product usage for each day within the specified time period.
    RETURN QUERY SELECT date_trunc('day', date_of_sale) AS date, COUNT(*) AS count
